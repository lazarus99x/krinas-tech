...
export const Reports: React.FC<ReportsProps> = ({ sales, isDarkMode, settings }) => {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `abr_inventory_sales_report_${timeRange}_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="hidden print:flex items-center justify-between mb-8 border-b pb-4">
        <div className="flex items-center gap-4">
           ...
           <div>
             <h1 className="text-xl font-bold text-gray-900">{settings?.companyName || 'ABR TECHNOLOGIES LIMITED'}</h1>
      ...
      <div className="hidden print:block text-center text-xs text-gray-400 mt-8">
        Generated by {settings?.companyName || 'ABR TECHNOLOGIES LIMITED'} on {new Date().toLocaleString()}
      </div>
