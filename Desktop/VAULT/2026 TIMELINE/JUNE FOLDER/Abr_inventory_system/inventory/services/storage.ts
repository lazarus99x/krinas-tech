...
const STORAGE_KEYS = {
  PRODUCTS: 'abr_products',
  CUSTOMERS: 'abr_customers',
  SALES: 'abr_sales',
  SETTINGS: 'abr_settings',
  USERS: 'abr_users',
  LOGS: 'abr_activity_logs',
  OFFLINE_QUEUE: 'abr_offline_action_queue',
  OFFLINE_SALES_LEGACY: 'abr_offline_sales_queue' // For migration
};

const OFFLINE_QUEUE_EVENT = 'abr:offline-queue-changed';
...
const INITIAL_USERS: User[] = [
  { id: 'u1', name: 'System Admin', email: 'admin@abrinventory.ng', password: 'password', role: 'ADMIN', isActive: true },
  { id: 'u2', name: 'John Sales', email: 'sales@abrinventory.ng', password: 'password', role: 'SALES_REP', isActive: true },
  { id: 'u3', name: 'Sarah Stock', email: 'stock@abrinventory.ng', password: 'password', role: 'INVENTORY_REP', isActive: true },
];
...
const INITIAL_SETTINGS: AppSettings = {
  companyName: 'ABR TECHNOLOGIES LIMITED',
  email: 'admin@abrinventory.ng',
...
const normalizeSettingsBranding = (settings: AppSettings): AppSettings => {
  if (settings.companyName === 'ABR INVENTORY' || settings.companyName === 'ABR Inventory System') {
    return { ...settings, companyName: 'ABR TECHNOLOGIES LIMITED' };
  }
  return settings;
};
  ...
  async getProducts(): Promise<Product[]> {
    if (isSupabaseConfigured()) {
      try {
        ...
        if (!error && data) {
           setLocalStorage(STORAGE_KEYS.PRODUCTS, data);
