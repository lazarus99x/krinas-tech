  ...
  try {
    ...
    for (const { symbol } of symbols) {
      try {
        ...
        const volumeChange = 0.95 + Math.random() * 0.1; // 0.95 - 1.05
        const volume24h = baseVolume * volumeChange;

        // Calculate high/low with small jitter window
        ...
        // Upsert market data (insert if missing)
        ...
        if (upsertError) {
          console.error(`Error updating ${symbol}:`, JSON.stringify(upsertError, null, 2), upsertError);
          continue;
        }
        ...
        if (historyError) {
          console.error(`Error adding price history for ${symbol}:`, JSON.stringify(historyError, null, 2));
        }

        updates.push({ symbol, price: newPrice });
      } catch (err: any) {
        console.error(`Market update failed for ${symbol}:`, err?.message || JSON.stringify(err));
      }
    }

    console.log(`✅ Market updated: ${updates.length} symbols`);
  } catch (error: any) {
